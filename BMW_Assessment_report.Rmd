...
This becomes evident when we verify the colors by component and the voltages that are used, being Sharpy Blue is the only color that is painted at voltages above 6000 V.  
...
df_45750_full_sensor %>% 
  ...
  count(color) %>% 
  left_join(df_colors, by=c("color"="color_code")) %>% 
  ggplot(aes(x = voltage, y = n, color=color_name,shape=component)) +
  geom_point() +
...
df_45750_full_sensor_model_1_train_2 %>% 
  ...
  kable(caption = "Porportion of correct/wrong predictions") %>%
  kable_classic(full_width = F, html_font = "Cambria") %>%
  kable_styling(latex_options = "HOLD_position")
...
df_45750_full_sensor_model_1_test_2 %>% 
  count(correct) %>% 
  mutate(proportion=round(n/sum(n),2)) %>% 
  kable(caption = "Porportion of correct/wrong predictions for testing data") %>%
  kable_classic(full_width = F, html_font = "Cambria") %>%
  kable_styling(latex_options = "HOLD_position")
...
where the most important result is the $p-values>0.05$ can be removed from the equation. Nevertheless, since is our first model, we will keep them.
...
[3] **The ABB Air Control Unit manual**